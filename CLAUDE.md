...
- Notable services: **`ctx.tier`** (`AccessManager`, [services/access.py](lncrawl/services/access.py))
  ...
  class dicts where `None` means unlimited. **`ctx.http`** (`FetchService`,
  [services/fetch.py](lncrawl/services/fetch.py)) — one shared `Scraper` behind an
  `EventLock`; use `with ctx.http.session(signal) as sess:` for all HTTP outside crawler
  sessions (translators, Calibre API, favicons). **`ctx.job_notifier`** — job-state emails,
  triggered automatically by handler helpers.
