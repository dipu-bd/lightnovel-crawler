...
- [lncrawl/__main__.py](lncrawl/__main__.py) → [lncrawl/app.py](lncrawl/app.py): Typer CLI. Subcommands `version`, `config`, `sources`, `crawl`, `search`, `server`, `discord`, `app`, plus hidden `dev`. `discord` starts a Discord bot (`lncrawl/discord_bot/`) with `/crawl`, `/search`, `/status` slash commands. `app` subcommand launches the desktop webview (`lncrawl/server/webview.py`). When running as a **frozen executable** (PyInstaller), `__main__.py` calls `webview.start()` directly, bypassing the CLI.
