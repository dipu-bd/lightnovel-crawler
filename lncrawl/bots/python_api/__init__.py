...
class PythonApiBot:
    ...
    def start_search(self, query: str) -> Dict[str, Any]:
        ...
        if not self.app:
            return {
                "status": "error",
                "message": "No active session. Call init_app first.",
            }
        ...
        try:
            ...
            if self.app.crawler:
                ...
                return {
                    "status": "success",
                    "message": "Direct novel URL found, no search needed",
                }
    ...
    def get_search_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_results": len(getattr(self.app, "search_results", [])) > 0
            or self.app.crawler is not None,
        }
    ...
    def get_search_results(self) -> Dict[str, Any]:
        ...
        for i, res in enumerate(self.app.search_results):
            ...
            results.append({"index": i, "title": res["title"], "sources": sources})
    ...
    def select_novel(self, novel_index: int, source_index: int = 0) -> Dict[str, Any]:
        ...
        if not hasattr(self.app, "search_results") or novel_index >= len(
        ...
        try:
            if selection_type == "all":
                ...
                if start is None or end is None:
                    return {
                        "status": "error",
                        "message": "Start and end required for range selection",
                    }
                ...
                if not volumes:
                    return {
                        "status": "error",
                        "message": "Volume list required for volume selection",
                    }
                self.app.chapters = [
                    chap
                    for chap in self.app.crawler.chapters
                    if volumes.count(chap["volume"]) > 0
                ]
            else:
                return {
                    "status": "error",
                    "message": f"Unknown selection type: {selection_type}",
                }

            return {"status": "success", "chapters_selected": len(self.app.chapters)}
        except Exception as e:
