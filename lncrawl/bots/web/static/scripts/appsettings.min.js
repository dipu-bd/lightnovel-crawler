(function (n, t) {
	...
	function setHtmlTagDarkMode(darkModeBtn, darkMode) {
		if (darkMode === !0) {
			$(".action-select a[data-tool='night-mode'] i").removeClass("icon-moon").addClass("icon-sun"),
			...
			$(".action-select a[data-tool='night-mode'] i").removeClass("icon-sun").addClass("icon-moon"),
	...
	r.fontPlusBtn.on("click", function () {
		var n = u.indexOf(parsedCookie.fontSize);
		n + 1 < u.length && (r.fontRangeInput.val(n + 1 + 1).trigger("input"),
			parsedCookie.fontSize = u[n + 1],
			o(parsedCookie.fontSize),
			setCookie())
	...
	return t.lnwSettingsFunc = e, e.init(), e
