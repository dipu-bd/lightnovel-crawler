...
# -*- coding: utf-8 -*-
"""
Crawler for [volarenovels.com](https://volarenovels.com/).
"""
import json
import logging
import re
...
logger = logging.getLogger('VOLARENOVELS')
...
class VolareNovelsCrawler(Crawler):
    ...
    def read_novel_info(self):
        ...
        if soup.select('div.entry-content p')[0].text.strip():
            self.novel_title = soup.select('div.entry-content p')[0].text.strip()
        else : 
        ...
        for item in soup.select('div.entry-content p'):
            if "raw" in item.text.strip().lower():
                raw = True

        if soup.find('hr').findNext('p').select('a'):
            chapters = soup.find('hr').findNext('p').select('a')
        elif raw:
            chapters = soup.select('div.entry-content p a')[1:]
        else : 
    ...
    def download_chapter_body(self, chapter):
        ...
        body = []
        contents = soup.select('div.entry-content p')[1:-1]
        for p in contents:
