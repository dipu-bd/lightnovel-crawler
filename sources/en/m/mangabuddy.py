...
class mangabuddyCrawler(Crawler):
    ...
    def read_novel_info(self):
        ...
        slug = re.search(fr'{self.home_url}(.*?)(/|\?|$)',
                         self.novel_url).group(1)
        ...
        self.novel_author = ', '.join(
            [
                a.text.strip()
                for a in soup.select(".detail a[href*='/authors/'] span")
            ]
        )
        logger.info('Novel author: %s', self.novel_author)

        soup = self.get_soup(f'{self.home_url}api/manga/{slug}'
    ...
    def download_chapter_body(self, chapter):
        ...
        main_server = re.search(r'var mainServer = "(.*)";',
                                server.text).group(1)
        img_list = re.search(r'var chapImages = \'(.*)\'',
                             images.text).group(1).split(',')

        image_urls = [f'<img src="{main_server}{img}">'
                      for img in img_list]
