...
class DiscordBot(discord.Client):
    ...
    async def on_ready(self):
        ...
        activity = discord.Game(name="🔥%slncrawl🔥" % signal)
    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, discord.abc.PrivateChannel):
            ...
            await self.send_public_text(
                message,
                'Enter `%slncrawl` to start a new session of **Lightnovel Crawler**' % signal
            )
        elif message.content == signal + 'lncrawl':
...
class MessageHandler:
    ...
    async def get_novel_info(self):
        ...
        self.state = self.report_novel_info_getter
        await self.download_novel_info()
    # end def

    async def report_novel_info_getter(self):
        await self.send('Getting information about your novel. Please wait...')
    # end def

    async def download_novel_info(self):
        async with self.user.typing():
    ...
    async def publish_files(self):
        try:
            ...
            download_url = '%s/%s' % (public_ip.strip('/'),
                                      str(self.user.id),
                                      self.app.good_file_name)
            ...
            for src in self.app.archived_outputs:
                ...
                shutil.copy(src, dst)
                urls.append(download_url + '/' + file_name)
            # end for
    ...
    async def report_download_progress(self):
        ...
        if text == '!cancel':
            ...
            self.destroy()
            await self.send('Session is now closed. Send *start* to create a new one.')
            return
        # end if
